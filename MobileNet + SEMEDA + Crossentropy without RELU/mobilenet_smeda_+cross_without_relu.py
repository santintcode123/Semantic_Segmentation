...
model1 = tf.keras.models.load_model('/content/drive/My Drive/Colab Notebooks/Edge_Net_without_RELU.h5')
model1.summary()
...
model.save("model_mobileNet_with_semeda_withoutRELU.h5")
...
imshow(X_train[122])
plt.show()
imshow(np.squeeze(Y_train[122]))
plt.show()
imshow(np.squeeze(preds_train_t[122]))
plt.show()
